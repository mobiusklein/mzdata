...
fn mzml_totaling(c: &mut Criterion) {
    c.bench_function("serial_execution", |b| b.iter(|| serial(black_box("./test/data/small.mzML"))));
    c.bench_function("serial_execution_with_index", |b| b.iter(|| serial_with_index(black_box("./test/data/small.mzML"))));
    c.bench_function("serial_with_external_iterator", |b| b.iter(|| serial_with_external_iterator(black_box("./test/data/small.mzML"))));
}
