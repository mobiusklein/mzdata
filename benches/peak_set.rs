...
use mzdata::peaks::{PeakSet, PeakCollection};
...
fn search(peak_set: &PeakSet, query: f64, error_tolerance: f64, error_type: MassErrorType) -> Option<usize> {
...
fn peak_search(c: &mut Criterion) {
    let peaks = MGFReader::new(fs::File::open("./test/data/small.mgf").unwrap()).next().unwrap().peaks;
    let queries: Vec<f64> = peaks.iter().map(|p|p.mz).collect();
    c.bench_function("peak_set_search", |b| b.iter(|| find_with_search(
        black_box(&peaks), black_box(&queries))
    ));
    c.bench_function("peak_set_find_inline", |b| b.iter(|| peak_set_find_inline(
        black_box(&peaks), black_box(&queries))
    ));
}
